...
from pdm_fancy_pypi_readme.__main__ import tomllib
from pdm_fancy_pypi_readme._cli import cli_run
...
def _pyproject():
    return tomllib.loads(Path("tests", "example_pyproject.toml").read_text())
...
class TestCLIEndToEnd:
    ...
    def test_ok(self):
        ...
        """
        out = run("pdm_fancy_pypi_readme", "tests/example_pyproject.toml")
...
class TestCLI:
    ...
    def test_cli_run_missing_config(self, capfd):
        ...
        Missing configuration is caught and gives helpful advice.
        """
        source = "`[tool.pdm.build.hooks.fancy-pypi-readme]` in pyproject.toml"
        with pytest.raises(SystemExit):
            cli_run(
                {"project": {"dynamic": ["foo", "readme", "bar"]}},
                sys.stdout,
